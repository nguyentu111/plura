...
const Checkout = (props: Props) => {
  ...
  useEffect(() => {
    if (funnelId) {
      const fetchData = async () => {
        ...
        console.log(JSON.parse(funnelData?.liveProducts || "[]"));
  ...
  return (
    <div
      ...
      className={clsx(
        "p-2 w-full relative text-[16px] transition-all flex items-center justify-center",
        {
          "!border-blue-500":
    ...
    >
      {!state.editor.liveMode && (
        <div className="absolute inset-0" onClick={handleOnClickBody}></div>
      )}
      {state.editor.selectedElement.id === props.element.id &&
        !state.editor.liveMode && (
      ...
      <div className="border-none transition-all w-full">
        <div className="flex flex-col gap-4 w-full">
          ...
          {!livePrices.length && !state.editor.liveMode && (
            <div className="">
              No product found. Enable products in funnel settings.
            </div>
          )}
          {!!livePrices.length && !options.clientSecret && (
